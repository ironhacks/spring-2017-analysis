...
```{r}
scores_loc <- read.table("/Users/jialincheoh/analysis-2017/scores/spring-2017-analysis/data/overall_score_click_result.csv", header=TRUE, sep = ",")
scores_loc
```
```{r}
...
# Variance is constant. One of ANOVA conditions fulfilled. 
...
```{r}
# Normality still violated after Box-Cox transformations
...
```{r}
noveltyF.mod <- lm(novelty.phase5 ~ as.factor(Group), scores_loc)
anova(lmnovelty, noveltyF.mod)
...
```{r}
# There is a linear impact of total.phase5 on Group
...
```
```{r}
totalF.mod <- lm(total.phase5 ~ as.factor(Group), scores_loc)
anova(lmtotal, totalF.mod)
