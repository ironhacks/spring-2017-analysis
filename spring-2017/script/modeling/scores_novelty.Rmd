...
```
```{r}
tableb <- aov(log_relational_novelty ~ Q7_Q7_1 + Q7_Q7_2 + Q8_Q8_1 + Q10 + log_count + group + log_improve_total, data = df)
summary(tableb)
```
```{r}
...
```
```{r}
mod4<- lmer(log_relational_novelty ~ Q7_Q7_1 + Q7_Q7_2 + Q8_Q8_1 + Q10 + log_count + group + log_improve_total + ( 1 | stage ), data = df, REML = FALSE )
summary(mod4)
```
