...
```{r}
df<-read.csv("input/overall_scores.csv", header =TRUE, sep=",")
...
cols <- c("Q7_Q7_1", "Q7_Q7_2", "Q8_Q8_1", "Q10", "log_novelty", "log_user_requirement", "log_infovis", "log_total")
...
```{r}
mod <- lm( log_novelty ~ factor(Group), data = df)
summary(mod)
```
```{r}
mod <- lm(log_user_requirement ~ factor(Group) , data = df)
summary(mod)
```
```{r}
mod <- lm( log_infovis ~ factor(Group) , data = df)
summary(mod)
```

```{r}
mod <- lm( log_total ~ factor(Group) + Q7_Q7_1 + Q7_Q7_2 + Q8_Q8_1 + Q10 , data = df)
summary(mod)
```
