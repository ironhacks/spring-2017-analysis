...
```
```{r}
regdata <- cbind(scores_loc[, c(3,4,6,86,87,88,89)], fa1$scores)

#Labeling the data

names(regdata) <- c("Group", "user.requirement.phase5", "novelty.phase5", "sum.clicks.count", "sum.add.loc", "project.clicks.count", "score.clicks.count", "intrinsic.motivation", "extrinsic.motivation")
head(regdata)
```
...
```{r}
library('relaimpo')
gpa.model<-lm(novelty.phase5~ Group, data=regdata)
summary(gpa.model)
```
